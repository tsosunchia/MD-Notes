...
正是由于可以使用**位运算**来代替算数运算的公式，效率更高，所以让下标从 1 开始。

- 左孩子：2 * i（i << 1）
- 右孩子：2 * i + 1（i << 1 | 1）
- 父节点：i / 2（i >> 1）

##### 如何：将数组转化成大根堆？这个过程叫 heapInsert 操作
...
##### 如何：返回并删除大根堆的最大值，剩余数字依然保持大根堆？这个过程叫 heapify 操作

将顶部元素取出；将大根堆的最后位置上的元素覆盖到顶部，然后while比较，如果左右孩子中较大的数比当前节点更大，则交换当前节点和较大的孩子节点，直到越界或左右孩子都不比当前节点大为止。

```java
// 返回最大值，并且在大根堆中，把最大值删掉。剩下的数，依然保持大根堆形态
public int pop() {
    int ans = heap[0];
...
// 从index位置，往下看，不断的下沉，直到：我的孩子都不再比我大，或者已经没孩子了，就停止
...
- 手写堆结构，因为增加了对象的位置表，所以能够满足动态改信息的resign需求（例如 Dijskra 会用到），而系统自带的堆结构即使是实现了对数据的修改，时间复杂度也不会好，因为它没有 hashmap，只能从顶部开始逐个进行 heapify

  ```java
  public void resign(T value) {
     int valueIndex = indexMap.get(value);
     heapInsert(valueIndex); // 这个
     heapify(valueIndex, heapSize); // 和上面的，只会命中一个，另一个直接返回
  }
  ```
